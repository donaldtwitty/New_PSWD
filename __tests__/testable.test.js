...
const { mobileNav, onScroll, handleFormSubmission, closePopup } = require('../New_PSWD/assets/js/custom-testable.js');
const { openDCModal, closeDCModal, handleClickOutside, handleEscapeKey } = require('../New_PSWD/assets/js/modal-testable.js');

describe('Modal Functions Coverage', () => {
  beforeEach(() => {
    document.body.innerHTML = `<div id="dc-modal" style="display: none;"></div>`;
  });
...
describe('Custom Functions Coverage', () => {
  ...
  test('mobileNav function runs with width < 992', () => {
    const mockJQuery = jest.fn((selector) => {
        ...
        return {
          on: jest.fn((event, callback) => {
            ...
            global.$ = jest.fn((innerSelector) => {
              ...
              return { find: jest.fn(() => ({ toggleClass: jest.fn() })) };
            });
            callback.call({});
      ...
      return { removeClass: jest.fn(), find: jest.fn(() => ({ toggleClass: jest.fn() })) };
    });
    global.$ = mockJQuery;
    expect(() => mobileNav()).not.toThrow();
  });
      ...
      if (selector === '.nav a') {
        return {
          each: jest.fn((callback) => {
            const mockThis = { attr: () => '#test', addClass: jest.fn(), removeClass: jest.fn() };
            const currLink = { attr: () => '#test', addClass: jest.fn(), removeClass: jest.fn() };
            mockJQuery.mockReturnValueOnce(currLink);
            mockJQuery.mockReturnValueOnce({ position: () => ({ top: 50 }), height: () => 100 });
            mockJQuery.mockReturnValueOnce({ removeClass: jest.fn() });
            callback.call(mockThis);
          })
        };
      }
      return { position: () => ({ top: 50 }), height: () => 100, removeClass: jest.fn() };
    });
    global.$ = mockJQuery;
